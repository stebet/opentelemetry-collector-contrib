...
package emasamplingprocessor // import "github.com/open-telemetry/opentelemetry-collector-contrib/processor/emasamplingprocessor"
...
var typeStr = component.MustNewType("ema_sampling")
...
// The accumulated field contains ONLY spans belonging to this trace ID.
type traceData struct {
	arrivedAt   time.Time
	accumulated ptrace.Traces // per-trace accumulator; spans filtered to this trace only
	spanCount   int64
}

// emaSamplingProcessor is a tail sampling processor that uses an Exponential
// Moving Average (EMA) algorithm to dynamically adjust sampling rates per key.
...
func newEMASamplingProcessor(
	set processor.Settings,
	nextConsumer consumer.Traces,
	cfg *Config,
) (*emaSamplingProcessor, error) {
...
func (p *emaSamplingProcessor) Start(_ context.Context, _ component.Host) error {
...
func (p *emaSamplingProcessor) Shutdown(ctx context.Context) error {
	...
	// Flush remaining traces with whatever data is available.
	p.mu.Lock()
	remaining := p.traces
...
// Each trace's accumulator contains ONLY spans belonging to that trace ID.
func (p *emaSamplingProcessor) ConsumeTraces(ctx context.Context, incoming ptrace.Traces) error {
	now := time.Now()
	...
	for i := 0; i < inRS.Len(); i++ {
		...
		for j := 0; j < inSS.Len(); j++ {
			...
			// Group span indices by trace ID within this scope.
			...
			for tid, indices := range byTrace {
				...
				// Append a new resource+scope block with only the filtered spans.
	...
	// Evict oldest traces if we exceed NumTraces.
	// Returns entries to decide synchronously after releasing the lock.
...
// drainOldest removes trace entries until len(p.traces) <= NumTraces and returns
// the removed entries so the caller can decide on them outside the lock.
// Must be called with p.mu held.
func (p *emaSamplingProcessor) drainOldest() map[pcommon.TraceID]*traceData {
...
func (p *emaSamplingProcessor) flushLoop() {
	defer p.wg.Done()
...
func (p *emaSamplingProcessor) flush(ctx context.Context) {
...
func (p *emaSamplingProcessor) decide(ctx context.Context, _ pcommon.TraceID, td *traceData) {
	...
	// Force-keep traces containing errors or exception events.
...
func (p *emaSamplingProcessor) traceHasErrors(td *traceData) bool {
...
func (p *emaSamplingProcessor) addSampleRateAttr(td *traceData, rate int) {
	rs := td.accumulated.ResourceSpans()
	for i := 0; i < rs.Len(); i++ {
		scopeSpans := rs.At(i).ScopeSpans()
		for j := 0; j < scopeSpans.Len(); j++ {
...
func (p *emaSamplingProcessor) buildSamplingKey(td *traceData) string {
...
func (p *emaSamplingProcessor) findAttributeValue(td *traceData, attr string) string {
	rs := td.accumulated.ResourceSpans()
	for i := 0; i < rs.Len(); i++ {
		resourceSpan := rs.At(i)

		// Check resource-level attributes first.
		if v, ok := resourceSpan.Resource().Attributes().Get(attr); ok {
			return v.AsString()
		}

		// Check span-level attributes.
